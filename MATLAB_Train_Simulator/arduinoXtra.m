classdef arduinoXtra < handle
    
    % This class defines an "arduinoXtra" object
    ...
    % modified on 111213 to flush serial per Tony Shoup (Lima Campus)
    ...
    methods
        ...
        function a=arduinoXtra(comPort)
        ...
        function val=analogRead(a,pin)
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Flush any chars in serial buffer before command
                flush(a);  % ALShoup 10/05/2013
        ...
        function randomGateLeds(a,val)
            
            % a.randomGateLeds(); Every 4 to 7 seconds a random timer
            % process will choose to turn on:
            % LED 6 or 7 (incoming) 
            % LED 8 or 9 (outgoing)
            % if 6 is turned on 7 is turned off and vice versa
            % if 8 is turned on 9 is turned off and vice versa
            %
            % Example:
            % a.randomGateleds();
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                if val==0
                    % send mode, value
                    fwrite(a.aser,[83,48],'uchar');
                else
                    % send mode, value
                    fwrite(a.aser,[83,49],'uchar');
                end
